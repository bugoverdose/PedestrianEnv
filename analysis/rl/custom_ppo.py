...
class CustomPPO(PPO):
    ...
    def train(self) -> None:
        ...
        decay_until_steps = int(self.n_epochs * self.ent_coef_fraction)
        decay_change = self.ent_coef_final - self.ent_coef_init
        # train for n_epochs epochs
        for epoch in range(self.n_epochs):
            # NOTE: linear decay
            if epoch > decay_until_steps:
                progress = (epoch / decay_until_steps)
                self.ent_coef = self.ent_coef_init - (progress * decay_change)
            approx_kl_divs = []

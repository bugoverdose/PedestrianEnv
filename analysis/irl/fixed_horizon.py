...
class FixedHorizonAbsorbIndicator(gym.Wrapper):
    ...
    def ongoing_obs_with_absorbing_channel(self, obs):
        assert obs.ndim == 3
        ind = np.full((1, obs.shape[1], obs.shape[2]), float(0.0), dtype=obs.dtype)
    ...
    def reset(self, **kw):
        ...
        return self.ongoing_obs_with_absorbing_channel(obs), self.last_info

    def step(self, action):
        self.cur_step += 1
        if not self.is_absorbing:
            obs, rew, terminated, truncated, info = self.env.step(action)
            obs = self.ongoing_obs_with_absorbing_channel(obs)
            self.update_last_ongoing_info(info)

...
class CraDrawer:
    ...
    def draw_main_before(self, block, n_trial):
        text = visual.TextStim(
            self.window,
            self.instructions['main_before'].format(block, n_trial),
            font=self.text_font,
            pos=(0, 0),
            wrapWidth=30,
            anchorVert='center')
    ...
    def draw_outro(self, block):
        text = visual.TextStim(
            self.window,
            self.instructions['outro'].format(block),
            font=self.text_font,
            pos=(0, 0),
            wrapWidth=30,
            anchorVert='center')
...
class CraRunner:
    ...
    #     pr_risky = [0.13,0.25,0.38] # np.linspace(0.1, 0.4, 11) 
    #     am_risky = np.array(0).reshape(-1)

    #     pr_ambig = np.array(0.5).reshape(-1)
    #     am_ambig = [0.25,0.5,0.75]# np.linspace(0.1, 0.9, 11) #np.linspace(0.0, 0.8, 11) #
    ...
    #     r_var = np.round(np.logspace(np.log10(5), np.log10(230), 11, base=10))
    #     r_fix = np.round(np.logspace(np.log10(5), np.log10(115), 11, base=10))
    ...
    def generate_fixed_designs():
        ...
        rval = np.array([5, 9.5, 18, 34, 65]) # rewards for variable option
        rfix = np.array([5])                  # rewards for option fixed to 50% chance

        rs = np.vstack([(v, f) for v in rval for f in rfix])

        designs = np.array([
            np.concatenate([pr_am[i], rs[j], [k]])
    ...
    def show_outro(self, block):
        self.drawer.draw_outro(block)
        self.window.flip()
